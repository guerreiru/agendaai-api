...
import { findAvailabilitiesByProfessionalAndWeekday } from "../repositories/availability.repository.js";
import { findCompanyById } from "../repositories/company.repository.js";
import { findProfessionalServiceByProfessionalAndService } from "../repositories/professional-service.repository.js";
...
export async function listAppointments(
  ...
  if (!context) {
    return findAppointments(filters);
  }
  ...
  if (context.actorRole === "CLIENT") {
    const where: Prisma.AppointmentWhereInput = { clientId: context.actorId };
    if (filters?.startDate || filters?.endDate) {
      where.date = {};
      if (filters.startDate) where.date.gte = filters.startDate;
      if (filters.endDate) where.date.lte = filters.endDate;
  ...
  if (context.actorRole === "PROFESSIONAL") {
    const where: Prisma.AppointmentWhereInput = {
      professionalId: context.actorId,
    };
    if (filters?.startDate || filters?.endDate) {
      where.date = {};
      if (filters.startDate) where.date.gte = filters.startDate;
...
export async function cancelAppointment(
  ...
  if (terminalStatuses.includes(existing.status)) {
    throw new AppError(
      "Cannot cancel an appointment that is already finalized.",
      400,
    );
  }
